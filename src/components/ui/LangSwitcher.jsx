  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen(o => !o)}
        ...
        style={{
          background: dark ? 'rgba(255,255,255,0.08)' : open ? '#eef0fc' : '#f8fafc',
          borderColor: dark ? 'rgba(255,255,255,0.15)' : open ? '#dee2f8' : '#e2e8f0',
          ...
          <div className="absolute right-0 top-full mt-1 rounded-xl overflow-hidden shadow-xl z-50"
            ...
            {LANGS.map(l => (
              <button key={l.code}
                ...
                style={{
                  background: lang === l.code ? '#eef0fc' : '#fff',
                  borderBottom: '1px solid #f1f5f9',
                }}>
                <span className="text-[16px]">{l.flag}</span>
                <div>
                  <div className="text-[12px] font-semibold" style={{color: lang===l.code ? '#5E6AD2' : '#1F1F1F'}}>
                    {l.name}
                  </div>
                </div>
