...
export default function CreateEstimateModal({ onClose, onCreated }) {
  ...
  const addProduct = (product) => {
    if (items.find(i => i.product_id === product.id)) {
      toast.error(`${product.name} is already in this estimate`)
      return
    }
    setItems([...items, {
  ...
  const create = async () => {
    ...
    if (error || !data?.success) {
      toast.error(error?.message || data?.message || 'Failed to create estimate')
      return
    }
      ...
      <div className="fixed inset-0 z-[400] flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.5)'}}>
        <div className="rounded-2xl overflow-hidden flex flex-col" style={{
          ...
          <div className="px-5 py-4 flex items-center justify-between flex-shrink-0" style={{borderBottom:'1px solid #E5E5E5'}}>
            <div>
              <div className="text-[11px] font-bold text-[#666] uppercase tracking-wider">New Estimate</div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            <div>
                  ...
                  <div className="bg-[#FAFAFA] border-t border-[#E5E5E5] px-4 py-3">
                    <div className="ml-auto max-w-[280px] text-[12px] space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="font-mono text-[#1F1F1F]">${totals.subtotal.toFixed(2)}</span>
                      </div>
                      {totals.discount > 0 && (
                        <div className="flex justify-between">
                      ...
                      <div className="flex justify-between pt-1.5 border-t border-[#E5E5E5]">
                        ...
                        <span className="font-mono text-[16px] font-bold text-[#1F1F1F]">${totals.total.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="mt-2 text-[11px] text-[#666] flex items-start gap-1.5 px-1">
                ...
                <span>Estimates do not affect inventory. Stock numbers shown are current levels for your reference.</span>
              </div>
            </div>
