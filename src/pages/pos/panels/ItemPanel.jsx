...
export default function ItemPanel({ item, onClose }) {
  ...
  return (
    <div className="w-[220px] bg-[#0d1117] border-l border-[#1e2d42] flex flex-col flex-shrink-0">
      {/* Header */}
      <div className="px-3 py-2.5 border-b border-[#1e2d42] flex items-center gap-2">
        <ProductPhoto
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[10px] font-mono text-blue-400">${lineTotal().toFixed(2)}</div>
        </div>
        ...
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`px-2.5 py-1.5 text-[10px] whitespace-nowrap border-b-2 cursor-pointer bg-transparent transition-all ${
              tab===t.id ? 'text-blue-400 border-blue-500' : 'text-[#3d5068] border-transparent'
            }`}>{t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        {tab === 'main' && (
          <div className="flex flex-col gap-3">
            {/* Qty */}
            <div>
              <div className="text-[9px] font-mono text-[#3d5068] uppercase mb-1.5">Quantity</div>
              <div className="flex items-center gap-2 mb-2">
                <button onClick={() => { const n=item.qty-1; setCustomQty(String(Math.max(0,n))); setItemQty(item.id, Math.max(0,n)) }}
                  className="w-8 h-8 bg-[#111827] border border-[#1e2d42] rounded-lg text-[16px] font-bold cursor-pointer hover:border-blue-500/40 hover:text-blue-400 transition-all flex items-center justify-center">−</button>
                <input
                  type="number" value={customQty}
                  onChange={e => handleQtyInput(e.target.value)}
                  className="flex-1 bg-[#111827] border border-[#1e2d42] rounded-lg px-2 py-1.5 text-[14px] font-mono font-bold text-center outline-none focus:border-blue-500/40"/>
                <button onClick={() => { const n=item.qty+1; setCustomQty(String(n)); setItemQty(item.id, n) }}
                  className="w-8 h-8 bg-[#111827] border border-[#1e2d42] rounded-lg text-[16px] font-bold cursor-pointer hover:border-blue-500/40 hover:text-blue-400 transition-all flex items-center justify-center">+</button>
              </div>
              <div className="text-[10px] text-[#3d5068] text-center">{item.unit || 'ea'} × ${item.unitPrice.toFixed(2)}</div>
            </div>
            ...
            {item.itemDiscount && (
              <div className="bg-pink-500/8 border border-pink-500/20 rounded-[9px] px-3 py-2 flex justify-between items-center">
                <div>
                  <div className="text-[9px] text-[#3d5068]">Item Discount</div>
                ...
                <button onClick={removeDiscount} className="text-[#3d5068] hover:text-red-400 bg-transparent border-none cursor-pointer text-[12px]">✕</button>
              </div>
            )}

            {/* Employee */}
            {item.employee && (
              <div className="bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 py-2 flex justify-between items-center">
                <div>
                  <div className="text-[9px] text-[#3d5068]">Staff</div>
                  <div className="text-[11px] font-bold">{item.employee.name}</div>
                </div>
                <button onClick={() => setItemEmployee(item.id, null)} className="text-[#3d5068] hover:text-red-400 bg-transparent border-none cursor-pointer text-[12px]">✕</button>
              </div>
            )}
            ...
            {item.priceOverridden && (
              <div className="bg-yellow-500/8 border border-yellow-500/20 rounded-[9px] px-3 py-2">
                <div className="text-[9px] text-[#3d5068]">Custom Price</div>
                <div className="text-[12px] font-bold text-yellow-400">${item.unitPrice.toFixed(2)}</div>
              </div>
            )}

            {/* Note */}
            {item.note && (
              <div className="bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 py-2">
                <div className="text-[9px] text-[#3d5068] mb-0.5">Note</div>
                <div className="text-[11px] text-[#8899b0]">{item.note}</div>
              </div>
            )}
            ...
            <button onClick={() => { removeItem(item.id); onClose() }}
              className="w-full bg-red-500/10 border border-red-500/20 rounded-[9px] py-2
                text-[11px] font-bold text-red-400 cursor-pointer hover:bg-red-500/15 transition-colors mt-1">
              🗑 Remove Item
            </button>
          </div>
        )}
          ...
          <div className="flex flex-col gap-3">
              ...
              {[['pct','%'],['amt','$']].map(([t,l]) => (
                <button key={t} onClick={() => setDiscType(t)}
                  className={`flex-1 py-2 rounded-lg text-[11px] font-bold border cursor-pointer transition-all ${
                    discType===t ? 'bg-pink-500/15 border-pink-500/40 text-pink-400' : 'bg-[#111827] border-[#1e2d42] text-[#8899b0]'
                  }`}>{l} {t==='pct'?'Percent':'Fixed'}</button>
              ))}
            </div>
            <div className="flex items-center bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 focus-within:border-pink-500/40">
              <span className="text-[#3d5068] mr-1 text-sm">{discType==='pct'?'%':'$'}</span>
              <input type="number" value={discVal} onChange={e=>setDiscVal(e.target.value)}
                placeholder="0" autoFocus
                className="flex-1 bg-transparent border-none outline-none py-2.5 text-[14px] font-mono placeholder-[#3d5068]"/>
            </div>
            {discVal && (
              <div className="bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 py-2 text-center">
                <div className="text-[9px] text-[#3d5068]">After discount</div>
            ...
            {item.itemDiscount && (
              <button onClick={removeDiscount}
                className="w-full bg-[#111827] border border-[#1e2d42] rounded-[9px] py-2 text-[11px] text-red-400 cursor-pointer">
                Remove Discount
              </button>
            )}
          </div>
        ...
        {tab === 'price' && (
          <div className="flex flex-col gap-3">
            <div className="text-[10px] text-[#3d5068] text-center">
              Original: <span className="text-[#8899b0] font-mono">${item.unitPrice.toFixed(2)}</span>
            </div>
            <div className="flex items-center bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 focus-within:border-yellow-500/40">
              ...
              <input type="number" value={newPrice} onChange={e=>setNewPrice(e.target.value)}
                step="0.01" autoFocus
                className="flex-1 bg-transparent border-none outline-none py-2.5 text-[16px] font-mono font-bold placeholder-[#3d5068]"/>
            </div>
            ...
            {item.priceOverridden && (
              <button onClick={() => { setItemPrice(item.id, item.originalPrice || item.unitPrice); setNewPrice(String(item.originalPrice || item.unitPrice)) }}
                className="w-full bg-[#111827] border border-[#1e2d42] rounded-[9px] py-2 text-[11px] text-[#8899b0] cursor-pointer">
                Reset to Original
              </button>
            )}
          </div>
        ...
        {tab === 'employee' && (
          <div className="flex flex-col gap-1.5">
            <div className="text-[9px] font-mono text-[#3d5068] uppercase mb-1">Select Staff</div>
            {staffList.map(s => (
              <button key={s.id} onClick={() => { setItemEmployee(item.id, { id: s.id, name: s.name }); setTab('main') }}
                className={`flex items-center gap-2.5 px-3 py-2 rounded-[9px] border cursor-pointer transition-all text-left ${
                  item.employee?.id===s.id
                    ...
                    : 'bg-[#111827] border-[#1e2d42] text-[#e8edf5] hover:border-[#243347]'
                }`}>
                <div className="w-6 h-6 rounded-full bg-[#1a2236] flex items-center justify-center text-[10px] font-bold flex-shrink-0">
                  {s.name.charAt(0)}
                </div>
                <div>
                  <div className="text-[12px] font-semibold">{s.name}</div>
                  <div className="text-[9px] text-[#3d5068]">{s.role}</div>
                </div>
                {item.employee?.id===s.id && <span className="ml-auto text-blue-400">✓</span>}
              </button>
            ))}
            {item.employee && (
              <button onClick={() => setItemEmployee(item.id, null)}
                className="w-full bg-[#111827] border border-[#1e2d42] rounded-[9px] py-2 text-[11px] text-red-400 cursor-pointer mt-1">
                Remove Staff
              </button>
            )}
          </div>
        ...
        {tab === 'note' && (
          <div className="flex flex-col gap-3">
            <textarea
              ...
              className="w-full bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 py-2.5 text-[12px] outline-none focus:border-blue-500/40 resize-none placeholder-[#3d5068]"/>
            <button onClick={saveNote}
              className="w-full bg-blue-500 border-none rounded-[9px] py-2.5 text-[12px] font-bold text-white cursor-pointer">
              ✓ Save Note
            </button>
            {note && (
