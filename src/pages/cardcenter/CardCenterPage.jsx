...
export default function CardCenterPage() {
  ...
  return (
    <div className="flex flex-col h-full bg-[#07090f]">

      {/* Header */}
      <div className="px-6 py-4 bg-[#0d1117] border-b border-[#1e2d42] flex items-center gap-4 flex-shrink-0">
        <div className="text-[18px] font-bold">💳 Card Center</div>

        {/* Tabs */}
        <div className="flex gap-1 bg-[#111827] border border-[#1e2d42] rounded-[7px] overflow-hidden">
          {[['transactions','Transactions'],['batches','Batch History']].map(([id,label]) => (
            <button key={id} onClick={() => setTab(id)}
              className={`px-3 py-1.5 text-[11px] transition-all ${
                tab === id ? 'bg-[#1a2236] text-white' : 'text-[#8899b0] hover:text-white'
              }`}>
              {label}
            </button>
          ))}
        ...
        <div className="flex gap-2 ml-4">
          ...
          <select value={terminalFilter} onChange={e => setTerminalFilter(e.target.value)}
            className="bg-[#111827] border border-[#1e2d42] rounded-lg px-2.5 py-1.5
              text-[11px] text-[#e8edf5] outline-none">
      ...
      {tab === 'transactions' && (
        <div className="grid grid-cols-4 border-b border-[#1e2d42] flex-shrink-0">
          {[
            ...
            ['Voided',     `$${totalVoided.toFixed(2)}`,   '#8899b0', transactions.filter(t=>t.status==='voided').length],
            ['Refunded',   `$${totalRefunded.toFixed(2)}`, '#14b8a6', transactions.filter(t=>['refunded','partially_refunded'].includes(t.status)).length],
          ].map(([l,v,c,count]) => (
            <div key={l} className="px-5 py-3 border-r border-[#1e2d42] last:border-0 bg-[#0d1117]">
              <div className="text-[9px] font-mono text-[#3d5068] uppercase tracking-wider mb-1">
                {l} ({count})
              </div>
      ...
      <div className="flex-1 overflow-auto">
              ...
              <tr className="bg-[#111827] border-b border-[#1e2d42]">
                {['Time','Terminal','Order','Card','Amount','Status','Actions'].map(h => (
                  <th key={h} className="px-4 py-2.5 text-left font-mono text-[10px]
                    text-[#3d5068] uppercase tracking-wider">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {isLoading
                ...
                : transactions.map(tx => {
                    ...
                    return (
                      <tr key={tx.id}
                        className="border-b border-[#1e2d42] hover:bg-[#111827] transition-colors">
                        <td className="px-4 py-3 text-[11px] text-[#8899b0] font-mono">
                          {format(new Date(tx.authorized_at), 'MM/dd HH:mm')}
                        </td>
                        <td className="px-4 py-3 text-[11px] text-[#8899b0]">
                            ...
                            {canVoid && (
                              <button
                                ...
                                className="bg-red-500/10 border border-red-500/20 rounded px-2 py-1
                                  text-[10px] text-red-400 hover:bg-red-500/15 transition-colors">
                                Void
                              </button>
                            )}
                            {canRefund && (
                              <button
                                ...
                                className="bg-green-500/10 border border-green-500/20 rounded px-2 py-1
                                  text-[10px] text-green-400 hover:bg-green-500/15 transition-colors">
                                Refund
                              </button>
                            )}
                            {!canVoid && !canRefund && (
                              <span className="text-[10px] text-[#3d5068]">—</span>
                            )}
                          </div>
                        </td>
                      </tr>
        ...
        {tab === 'batches' && (
          <table className="w-full border-collapse">
            ...
            <tbody>
              {batches.map(batch => (
                <tr key={batch.id} className="border-b border-[#1e2d42] hover:bg-[#111827]">
                  <td className="px-4 py-3 font-mono text-[11px]">
                    {format(new Date(batch.closed_at), 'MMM d, yyyy HH:mm')}
                  </td>
                  <td className="px-4 py-3 text-[11px] text-[#8899b0]">{batch.terminal_name || '—'}</td>
                  ...
                  <td className="px-4 py-3 font-mono text-[12px] text-green-400">${batch.total_sales?.toFixed(2)}</td>
                  <td className="px-4 py-3 font-mono text-[12px] text-red-400">-${batch.total_refunds?.toFixed(2)}</td>
                  <td className="px-4 py-3 font-mono text-[12px] text-[#8899b0]">-${batch.total_voids?.toFixed(2)}</td>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[9px] font-mono px-2 py-0.5 rounded ${
                      batch.status === 'success' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'
                    }`}>
          ...
          <div className="bg-[#0d1117] border border-red-500/30 rounded-2xl w-[380px] p-6">
            {!pinFor || !needsVoidAuth ? (
              <>
                ...
                <div className="bg-[#111827] border border-[#1e2d42] rounded-[9px] p-3.5 mb-5">
              ...
              <>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleBatchClose()}
                    className="flex-[2] bg-gradient-to-r from-blue-600 to-blue-700 border-none rounded-[9px] py-2.5 text-[13px] font-bold text-white">
                    ✓ Close Batch
                  </button>
                </div>
              </>
...
function PinInput({ title, subtitle, value, onChange, onConfirm, onCancel }) {
  return (
    <div className="text-center">
      ...
      <div className="text-[11px] text-[#8899b0] mb-4">{subtitle}</div>
        ...
        {[1,2,3,4,5,6,7,8,9,'',0,'⌫'].map((k,i) => (
          <button key={i}
            ...
            className={`py-2.5 rounded-lg font-mono text-[15px] font-bold transition-all
              ${k==='' ? 'invisible' : 'bg-[#111827] border border-[#1e2d42] hover:bg-[#1a2236]'}`}>
            {k}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onConfirm} disabled={value.length < 4}
          className="flex-[2] bg-blue-500 border-none rounded-[9px] py-2.5 text-[13px] font-bold text-white disabled:opacity-40">
          Authorize
        </button>
      </div>
    </div>
