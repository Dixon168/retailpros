      ...
      {tab === 'overview' && (
        <div>
          <div className="grid grid-cols-5 gap-3 mb-6">
            {[
              ['Active Cards', cards.filter(c=>c.status==='active').length, '#5E6AD2'],
      ...
      {tab === 'cards' && (
        <div>
          <div className="flex gap-4 mb-4">
            {[['Member Cards', memberCards.length, '#5E6AD2'],['Gift Cards', giftCards.length, '#10b981']].map(([l,v,c]) => (
            ...
            {cards.map(card => (
              <div key={card.id} className="grid border-b border-[#E5E5E5] last:border-0 hover:bg-[#F5F5F5] transition-colors" style={{gridTemplateColumns:'1.5fr 1fr 1fr 1fr 80px'}}>
                <div className="px-3.5 py-3 flex items-center gap-2">
                  <span className="font-mono text-[12px] font-bold" style={{color: card.type==='member'?'#5E6AD2':'#10b981'}}>{card.card_number}</span>
                  <span className="text-[8px] font-mono px-1.5 py-0.5 rounded font-bold" style={{background: card.type==='member'?'rgba(139,92,246,0.12)':'rgba(16,185,129,0.12)', color: card.type==='member'?'#5E6AD2':'#10b981'}}>
