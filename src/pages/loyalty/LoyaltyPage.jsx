...
export default function LoyaltyPage() {
  ...
  return (
    <div className="p-6 overflow-y-auto h-full bg-[#07090f]">
      ...
      {/* Tabs */}
      <div className="flex gap-1 border-b border-[#1e2d42] mb-5">
        {TABS.map(t => (
          <button key={t.id} onClick={()=>setTab(t.id)}
            className={`px-4 py-2.5 text-[12px] border-b-2 transition-all ${tab===t.id?'text-yellow-400 border-yellow-400':'text-[#8899b0] border-transparent hover:text-white'}`}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        <div>
          ...
          <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] overflow-hidden">
            {topCustomers.slice(0,5).map((c,i) => (
              <div key={c.id} className="flex items-center gap-4 px-4 py-3 border-b border-[#1e2d42] last:border-0 hover:bg-[#111827] transition-colors">
                <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold ${i===0?'bg-yellow-500/20 text-yellow-400':i===1?'bg-slate-500/20 text-slate-300':i===2?'bg-orange-500/20 text-orange-400':'bg-[#1a2236] text-[#3d5068]'}`}>{i+1}</div>
                ...
                <div className="font-mono text-[11px] text-green-400">${(c.loyalty_points/100).toFixed(2)}</div>
                <div className="font-mono text-[11px] text-[#8899b0]">${c.total_spent?.toFixed(0)||'0'} total</div>
              </div>
            ))}
            {topCustomers.length === 0 && <div className="text-center py-8 text-[#3d5068] text-sm">No loyalty members yet</div>}
          </div>
        </div>
      )}
            ...
            {[['Member Cards', memberCards.length, '#006AFF'],['Gift Cards', giftCards.length, '#10b981']].map(([l,v,c]) => (
              <div key={l} className="flex-1 bg-[#0d1117] border border-[#1e2d42] rounded-[10px] p-3.5 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-[10px] text-[#3d5068]">{l}</div>
                </div>
              </div>
            ))}
          </div>
          <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] overflow-hidden">
            <div className="grid border-b border-[#1e2d42] bg-[#111827]" style={{gridTemplateColumns:'1.5fr 1fr 1fr 1fr 80px'}}>
            ...
            {cards.map(card => (
              <div key={card.id} className="grid border-b border-[#1e2d42] last:border-0 hover:bg-[#111827] transition-colors" style={{gridTemplateColumns:'1.5fr 1fr 1fr 1fr 80px'}}>
                ...
                <div className="px-3.5 py-3">
                  <span className={`text-[9px] font-mono px-2 py-0.5 rounded ${card.status==='active'?'bg-green-500/10 text-green-400':'bg-[#1a2236] text-[#8899b0]'}`}>
                    {card.status?.toUpperCase()}
                  </span>
                </div>
                <div className="px-3.5 py-3 flex gap-1.5">
                  <button onClick={()=>{setSelectedCard(card);setPayAmt('');setFaceAmt('');setShowRecharge(true)}}
                    className="bg-green-500/10 border border-green-500/20 rounded px-2 py-1 text-[10px] text-green-400 font-bold">+$</button>
                  <button onClick={()=>toast.success('Card history')} className="bg-[#111827] border border-[#1e2d42] rounded px-2 py-1 text-[10px] text-[#8899b0]">Hist</button>
                </div>
              </div>
            ))}
            {cards.length === 0 && <div className="text-center py-8 text-[#3d5068] text-sm">No cards issued yet</div>}
          </div>
        </div>
      )}
      ...
      {tab === 'leaderboard' && (
        <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] overflow-hidden">
          <div className="grid border-b border-[#1e2d42] bg-[#111827]" style={{gridTemplateColumns:'50px 2fr 1fr 1fr 1fr 1fr'}}>
            {['#','Customer','Points','Value','Spent','Tier'].map(h => (
              <div key={h} className="px-3.5 py-2.5 font-mono text-[10px] text-[#3d5068] uppercase tracking-wider">{h}</div>
            ))}
          </div>
          {topCustomers.map((c,i) => (
            <div key={c.id} className="grid border-b border-[#1e2d42] last:border-0 hover:bg-[#111827] transition-colors" style={{gridTemplateColumns:'50px 2fr 1fr 1fr 1fr 1fr'}}>
              <div className="px-3.5 py-3">
                <div className={`w-6 h-6 rounded-md flex items-center justify-center text-[10px] font-bold ${i===0?'bg-yellow-500/20 text-yellow-400':i===1?'bg-slate-500/20 text-slate-300':i===2?'bg-orange-500/20 text-orange-400':'bg-[#1a2236] text-[#3d5068]'}`}>{i+1}</div>
              </div>
              ...
              <div className="px-3.5 py-3 font-mono text-[12px] text-green-400">${(c.loyalty_points/100).toFixed(2)}</div>
      ...
      {tab === 'settings' && (
        <div className="max-w-[600px]">
          <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] p-5 mb-4">
            <div className="flex justify-between mb-4">
              <div className="text-[13px] font-bold">🏷️ Points Earning Rules</div>
              <button onClick={()=>toast.success('Edit rules')} className="bg-[#111827] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-[11px] text-[#8899b0] hover:border-yellow-500/30 hover:text-yellow-400 transition-all">Edit</button>
            </div>
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[['Retail','1 pt','#10b981'],['Wholesale','0.5 pt','#06b6d4'],['VIP ⭐','2 pt','#f59e0b']].map(([type,pts,c]) => (
                <div key={type} className="bg-[#111827] border border-[#1e2d42] rounded-[10px] p-3.5 text-center">
                  ...
                  <div className="text-[10px] text-[#3d5068] mt-1">per $1 spent</div>
                </div>
              ))}
            </div>
            <div className="bg-[#111827] border border-[#1e2d42] rounded-[10px] p-3.5 flex gap-6">
              {[['100 pts','= $1.00 discount'],['Min Redeem','100 points'],['Expiry','Never']].map(([v,l]) => (
                <div key={l} className="text-center flex-1">
                  ...
                  <div className="text-[10px] text-[#3d5068] mt-1">{l}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] p-5">
            <div className="flex justify-between mb-4">
              ...
              <button onClick={()=>toast.success('Add promo')} className="bg-[#111827] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-[11px] text-[#8899b0] hover:border-yellow-500/30 hover:text-yellow-400 transition-all">+ Add</button>
            </div>
            {[['Pay $80 → Get $100','Always active','25% bonus'],['Pay $200 → Get $250','VIP only','25% bonus']].map(([name,scope,bonus]) => (
              <div key={name} className="flex items-center gap-3 bg-[#111827] border border-green-500/20 rounded-[9px] px-3.5 py-3 mb-2">
                <div className="text-lg">🌟</div>
                <div className="flex-1"><div className="text-[12px] font-bold">{name}</div><div className="text-[10px] text-[#3d5068] mt-0.5">{scope} · {bonus}</div></div>
        ...
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.4)] backdrop-blur-sm z-50 flex items-center justify-center" onClick={()=>setShowRecharge(false)}>
          <div className="bg-[#0d1117] border border-[#243347] rounded-2xl w-[400px]" onClick={e=>e.stopPropagation()}>
            <div className="px-5 py-4 border-b border-[#1e2d42]">
              ...
              <div className="text-[11px] font-mono text-[#3d5068] mt-1">{selectedCard.card_number} · Balance: ${selectedCard.balance?.toFixed(2)}</div>
            </div>
            <div className="p-5">
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                <div>
                  <div className="text-[11px] font-bold text-[#8899b0] uppercase tracking-wider mb-2">Customer Pays</div>
                  <input value={payAmt} onChange={e=>setPayAmt(e.target.value)} type="number" className="w-full bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 py-2.5 text-[14px] font-mono outline-none text-right focus:border-yellow-500/40" placeholder="80.00"/>
                </div>
                <div>
                  <div className="text-[11px] font-bold text-[#8899b0] uppercase tracking-wider mb-2">Card Receives</div>
                  <input value={faceAmt} onChange={e=>setFaceAmt(e.target.value)} type="number" className="w-full bg-[#111827] border border-green-500/30 rounded-[9px] px-3 py-2.5 text-[14px] font-mono outline-none text-right" placeholder="100.00"/>
                </div>
              </div>
              <div className="flex gap-1.5 mb-4">
                {[[40,50],[80,100],[160,200]].map(([p,f]) => (
                  <button key={p} onClick={()=>{setPayAmt(p);setFaceAmt(f)}} className="flex-1 bg-[#111827] border border-[#1e2d42] rounded-lg py-2 text-[10px] font-mono text-[#8899b0] hover:border-yellow-500/30 hover:text-yellow-400 transition-all">
                    ${p}→${f}
                  </button>
                ))}
              </div>
              {faceAmt && payAmt && parseFloat(faceAmt) > parseFloat(payAmt) && (
                <div className="bg-green-500/8 border border-green-500/20 rounded-[9px] px-3.5 py-2.5 mb-4 text-[11px] text-green-400">
              ...
              <div className="flex gap-2">
                <button onClick={()=>setShowRecharge(false)} className="flex-1 bg-[#111827] border border-[#1e2d42] rounded-[9px] py-2.5 text-[13px] text-[#8899b0]">Cancel</button>
