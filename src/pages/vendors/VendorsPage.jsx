...
const PO_STATUS = {
  draft:     { bg:'rgba(61,80,104,0.2)',   color:'#8899b0' },
    ...
    <div className="flex h-full bg-[#07090f]">
      ...
      <div className="w-[280px] bg-[#0d1117] border-r border-[#1e2d42] flex flex-col flex-shrink-0">
        <div className="p-3.5 border-b border-[#1e2d42]">
          <div className="flex items-center gap-2 bg-[#111827] border border-[#1e2d42] rounded-[9px] px-3 focus-within:border-orange-500/30 transition-colors">
            <span className="text-[#3d5068]">🔍</span>
            <input value={search} onChange={e=>setSearch(e.target.value)}
              placeholder="Search vendors..."
              className="bg-transparent border-none outline-none py-2.5 text-[12px] text-[#e8edf5] flex-1 font-sans placeholder-[#3d5068]"/>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {filtered.map((s,i) => (
            <div key={s.id} onClick={()=>setSelectedVendor(s)}
              className={`p-3 rounded-[10px] cursor-pointer border mb-1 transition-all ${selectedVendor?.id===s.id?'bg-[#111827] border-orange-500/40':'border-transparent hover:bg-[#111827]'}`}>
              <div className="flex items-center gap-2.5 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[10px] text-[#3d5068] mt-0.5">{s.email||s.phone||'—'}</div>
                </div>
              </div>
              <div className="flex gap-1.5 pl-[46px]">
                {s.payment_terms && <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-yellow-500/10 text-yellow-400">{s.payment_terms}</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedVendor ? (
        <div className="flex-1 overflow-y-auto p-6 bg-[#07090f]">
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex gap-2 mt-2">
                {selectedVendor.payment_terms && <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-yellow-500/10 text-yellow-400">{selectedVendor.payment_terms}</span>}
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-green-500/10 text-green-400">Active</span>
              </div>
            </div>
            <div className="flex gap-2">
              <button onClick={()=>toast.success('Edit vendor')} className="bg-[#111827] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-[11px] text-[#8899b0] hover:border-blue-500/30 hover:text-blue-400 transition-all">Edit</button>
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] p-4">
              <div className="text-[11px] font-bold text-[#8899b0] uppercase tracking-wider mb-3">Contact</div>
              {[
            ...
            <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] p-4">
              <div className="text-[11px] font-bold text-[#8899b0] uppercase tracking-wider mb-3">Payment Terms</div>
              {[
                ...
                <div key={l} className="flex justify-between mb-2 last:mb-0">
                  <span className="text-[11px] text-[#3d5068]">{l}</span>
                  <span className="text-[12px] font-semibold text-right max-w-[60%] text-[#8899b0]">{v}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="mb-2 flex justify-between items-center">
            ...
            <button onClick={()=>toast.success('New PO')} className="bg-[#111827] border border-[#1e2d42] rounded-lg px-3 py-1.5 text-[11px] text-[#8899b0] hover:border-orange-500/30 hover:text-orange-400 transition-all">+ New PO</button>
          </div>
          <div className="bg-[#0d1117] border border-[#1e2d42] rounded-[12px] overflow-hidden">
            <div className="grid border-b border-[#1e2d42] bg-[#111827]"
            ...
            {pos.length === 0 ? (
              <div className="text-center py-8 text-[#3d5068] text-sm">No purchase orders yet</div>
            ) : pos.map(po => {
              const st = PO_STATUS[po.status]||PO_STATUS.draft
              return (
                <div key={po.id} className="grid border-b border-[#1e2d42] last:border-0 hover:bg-[#111827] transition-colors cursor-pointer"
                  style={{gridTemplateColumns:'1.2fr 1fr 1fr 1fr 1fr 100px'}}>
                  <div className="px-3.5 py-3 font-mono text-[12px] font-bold text-blue-400">{po.po_number}</div>
                  ...
                  <div className="px-3.5 py-3 text-[11px] text-[#8899b0]">{po.order_date?new Date(po.order_date).toLocaleDateString():'—'}</div>
                  <div className="px-3.5 py-3 text-[11px] text-[#8899b0]">{po.expected_date?new Date(po.expected_date).toLocaleDateString():'—'}</div>
                  ...
                  <div className="px-3.5 py-3 flex gap-1.5">
                    <button onClick={()=>toast.success(`Opening ${po.po_number}`)} className="bg-[#111827] border border-[#1e2d42] rounded px-2 py-1 text-[10px] text-[#8899b0] hover:text-blue-400 transition-all">View</button>
                    {['ordered','partial'].includes(po.status) && (
                      <button onClick={()=>toast.success('Receive stock')} className="bg-green-500/10 border border-green-500/20 rounded px-2 py-1 text-[10px] text-green-400">Recv</button>
                    )}
                  </div>
                </div>
              )
