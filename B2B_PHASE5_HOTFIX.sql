-- ════════════════════════════════════════════════════════════════════
-- B2B PHASE 5 HOTFIX — Drop stale function overloads
-- ════════════════════════════════════════════════════════════════════
-- Problem: CREATE OR REPLACE FUNCTION only replaces if the parameter
-- signature is identical. Adding p_delivery_notes created a NEW overload
-- alongside the old one, and PostgREST gets confused about which to call.
-- This script drops ALL overloads and re-creates a single canonical version.
-- ════════════════════════════════════════════════════════════════════

-- ── PART 1: Drop ALL overloads of the affected functions ───────────
-- (The convert function depends on invoice_atomic, so drop it first.)
DO $hotfix$
DECLARE r RECORD;
BEGIN
  FOR r IN
    ...
    FROM pg_proc
    WHERE proname IN (
      'fn_convert_estimate_to_invoice',
      'fn_create_invoice_atomic',
      'fn_create_estimate_atomic'
    )
  LOOP
...
-- ── PART 2: Recreate fn_create_estimate_atomic (canonical, with p_delivery_notes) ─
CREATE FUNCTION fn_create_estimate_atomic(
  p_tenant_id     UUID,
  p_store_id      UUID,
  p_customer_id   UUID,
  p_valid_until   DATE,
...
-- ── PART 3: Recreate fn_create_invoice_atomic (canonical) ─────────
...
-- ── PART 4: Recreate fn_convert_estimate_to_invoice (carry delivery_notes) ─
CREATE FUNCTION fn_convert_estimate_to_invoice(
  p_tenant_id   UUID,
  p_estimate_id UUID,
  p_due_date    DATE,
  p_user_id     UUID
...
BEGIN
  ...
  v_result := fn_create_invoice_atomic(
    p_tenant_id,
    v_est.store_id,
    v_est.business_customer_id,
    p_due_date,
    v_est.notes,
    v_est.internal_notes,
    p_user_id,
    v_items,
    v_est.billing_address_snapshot,
    v_est.shipping_address_snapshot,
    p_estimate_id,
    v_est.delivery_notes
  );
...
-- ── PART 5: Force PostgREST to reload its schema cache ─────────────
NOTIFY pgrst, 'reload schema';


-- ── VERIFY: Exactly ONE row per function, all with p_delivery_notes ──
SELECT proname AS function_name,
       pg_get_function_arguments(oid) AS arguments
  FROM pg_proc
 WHERE proname IN ('fn_create_invoice_atomic',
                   'fn_create_estimate_atomic',
                   'fn_convert_estimate_to_invoice')
 ORDER BY proname;
